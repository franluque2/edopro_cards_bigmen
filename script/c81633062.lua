...
local ARCHETYPE=0x526

--add the conditions for the archetype swap here
function s.ED(c)
  return c:IsType(TYPE_XYZ) or c:IsType(TYPE_SYNCHRO) or c:IsType(TYPE_LINK)
end

function s.Backrow(c)
  return c:IsSpellTrap() and c:IsSetCard(0x526)
end
...
function s.op(e,tp,eg,ep,ev,re,r,rp)
	if e:GetLabel()==0 then
        ...
        e8:SetValue(SET_BATTLIN_BOXING)
...
function s.flipop(e,tp,eg,ep,ev,re,r,rp)
    ...
    local g=Duel.GetMatchingGroup(s.ED, tp, LOCATION_ALL, LOCATION_ALL, nil)
