--Gatling the Shadow Flower Shinobi (CT)
local s,id=GetID()
...
function s.filter(c)
	return c:IsPosition(POS_FACEUP_ATTACK) and c:IsLevelBelow(8) or c:IsRankBelow(8) or c:IsLinkBelow(4) and not c:IsMaximumModeSide()
end
