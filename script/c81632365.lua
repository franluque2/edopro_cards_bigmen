--Durga the Shadow Flower Stealth Submarine (CT)
local s,id=GetID()
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local sg=Duel.GetMatchingGroup(s.LowerLevelFilter,tp,0,LOCATION_MZONE,nil)
...
function s.LowerLevelFilter(c)
    return c:IsFaceup() and (c:IsLevelAbove(6) or c:IsRankAbove(6) or c:IsLinkAbove(3))
end