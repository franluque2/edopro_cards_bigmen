...
function s.initial_effect(c)
	--Activate Skill
	aux.AddSkillProcedure(c,2,true,nil,nil)
...
function s.op(e,tp,eg,ep,ev,re,r,rp)
	if e:GetLabel()==0 then
        ...
        e10:SetCondition(function (ef) return Duel.IsBattlePhase() end)
...
function s.negcon(e,tp,eg,ep,ev,re,r,rp)
    local cardid=Duel.GetChainInfo(0, CHAININFO_TRIGGERING_CODE)
    local cardeff=Duel.GetChainInfo(0,CHAININFO_TRIGGERING_EFFECT)
	return not (Duel.GetCurrentChain()>0 and cardid==MAN_WITH_THE_MARK and cardeff:GetCode()==EVENT_SUMMON_SUCCESS)
end
