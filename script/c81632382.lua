...
function s.damop(e,tp,eg,ep,ev,re,r,rp)
	...
	if ct>0 and Duel.Damage(1-tp,ct*100,REASON_EFFECT)>0
		and Duel.IsExistingMatchingCard(aux.FaceupFilter(aux.TRUE),tp,0,LOCATION_MZONE,1,nil)
		and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
		local dg=Duel.SelectMatchingCard(tp,aux.FaceupFilter(aux.TRUE),tp,0,LOCATION_MZONE,1,1,nil)
		if #dg>0 then
