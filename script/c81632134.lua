--Laundering General - Big Tidy (CT)
local s,id=GetID()
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsPlayerCanDiscardDeckAsCost(tp,1) and Duel.IsPlayerCanDiscardDeckAsCost(1-tp,1) and
	Duel.IsPlayerCanDraw(1-tp,1) and (Duel.GetFieldGroupCount(tp,0,LOCATION_HAND)>0) end
end

function s.operation(e,tp,eg,ep,ev,re,r,rp)
	--Requirement
	if Duel.ChangePosition(e:GetHandler(), POS_FACEUP_DEFENSE,POS_FACEUP_ATTACK,POS_FACEUP_DEFENSE,POS_FACEUP_ATTACK)<0 then return end
	--Effect
