...
function s.actlimit(e,te,tp)
	return te:IsHasType(EFFECT_TYPE_ACTIVATE) and te:IsActiveType(TYPE_SPELL+TYPE_TRAP) and not te:IsActiveType(TYPE_FIELD)
end
function s.filter(c,tp)
	return c:IsFaceup() and c:IsControler(1-tp) and c:IsPreviousLocation(LOCATION_EXTRA)
end
