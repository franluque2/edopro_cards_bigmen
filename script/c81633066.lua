--Conscription of the Phantom Beasts
...
function s.PhantomBeasts(c)
  return c:IsSetCard(0x1b)
end

function s.PBTargets(c)
  return c:IsCode(511002442, 511000711, 511002441, 23076639, 55461744, 77207191)
end

function s.L4PB(c)
  return (c:IsOriginalSetCard(0x1b) and c:IsLevelBelow(4)) or c:IsCode(23076639)
end

function s.BerfTargets(c)
  return c:IsCode(511002442, 511000711, 511002441, 70969517, 55461744)
end

function s.Horn(c)
  return c:IsCode(21350571)
end
...
function s.op(e,tp,eg,ep,ev,re,r,rp)
	if e:GetLabel()==0 then
        ...
        e5:SetTargetRange(LOCATION_DECK+LOCATION_GRAVE,0)
...
function s.flipop(e,tp,eg,ep,ev,re,r,rp)
    ...
    local g=Duel.GetMatchingGroup(s.PhantomBeasts, tp, LOCATION_ALL, LOCATION_ALL, nil)
    ...
    g=Duel.GetMatchingGroup(s.PBTargets, tp, LOCATION_ALL, LOCATION_ALL, nil)
    ...
    g=Duel.GetMatchingGroup(s.L4PB, tp, LOCATION_ALL, LOCATION_ALL, nil)
    ...
    g=Duel.GetMatchingGroup(s.BerfTargets, tp, LOCATION_ALL, LOCATION_ALL, nil)
    ...
    g=Duel.GetMatchingGroup(s.Horn, tp, LOCATION_ALL, LOCATION_ALL, nil)
