--Conscription of the Burning Skull
...
local ARCHETYPE=0x39

--add the conditions for the archetype swap here
function s.Skulls(c)
  return c:IsCode(99899504, 63804806, 26293219)
end

function s.LavalSynchros(c)
    return c:IsSetCard(0x39) and c:IsType(TYPE_SYNCHRO)
end

function s.SonicSkull(c)
    return c:IsCode(63804806)
end
...
function s.flipop(e,tp,eg,ep,ev,re,r,rp)
    ...
    local g=Duel.GetMatchingGroup(s.Skulls, tp, LOCATION_ALL, LOCATION_ALL, nil)
    ...
    g=Duel.GetMatchingGroup(s.SonicSkull, tp, LOCATION_ALL, LOCATION_ALL, nil)
    ...
    g=Duel.GetMatchingGroup(s.LavalSynchros, tp, LOCATION_ALL, LOCATION_ALL, nil)
