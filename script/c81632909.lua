...
local register=function(what)
	return function(...)
		local params={...}
		local tp=params[1]
		if not Duel.IsExistingMatchingCard(s.number_seven_filter,tp,LOCATION_MZONE,0,1,nil) then return what(...) end
...
function s.dicecon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetFlagEffect(tp,id)>0 and Duel.IsExistingMatchingCard(s.number_seven_filter,tp,LOCATION_MZONE,0,1,nil) and Duel.GetFlagEffectLabel(tp,id)>0
end
