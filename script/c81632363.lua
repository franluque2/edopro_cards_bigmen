...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 then
		if Duel.SendtoHand(g,nil,REASON_EFFECT)>0 then
			...
			if Duel.IsExistingMatchingCard(s.LowerLevelFilter(c),tp,0,LOCATION_MZONE,1,nil) then
				local e1=Effect.CreateEffect(c)
				...
				c:RegisterEffect(e1)
				local ag=Duel.GetMatchingGroup(s.LowerLevelFilter(c),tp,0,LOCATION_MZONE,nil)
...
function s.LowerLevelFilter(c)
    return c:IsFaceup() and (c:IsLevel(8) or c:IsRank(8) or c:IsLink(4))
end