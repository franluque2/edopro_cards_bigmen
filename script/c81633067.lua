--Conscription of the Insect Princess
...
function s.Names(c)
  return c:IsCode(37957847, 26185991, 511001351)
end

function s.Prickle(c)
  return c:IsCode(91559748)
end
...
function s.op(e,tp,eg,ep,ev,re,r,rp)
	if e:GetLabel()==0 then
        ...
        e7:SetTargetRange(LOCATION_HAND+LOCATION_DECK,0)
        e7:SetTarget(function(_,c)  return c:IsHasEffect(id+1) end)
        e7:SetValue(RACE_INSECT)
...
function s.flipop(e,tp,eg,ep,ev,re,r,rp)
    ...
    local g=Duel.GetMatchingGroup(s.Names, tp, LOCATION_ALL, LOCATION_ALL, nil)
    ...
    g=Duel.GetMatchingGroup(s.Prickle, tp, LOCATION_ALL, LOCATION_ALL, nil)
